...
public class UserCredentialQueryService(IQueryService<UserCredential, int> q) : IUserCredentialQueryService
{
    ...
    public Task<UserCredential?> GetByNormalizedLogin(string normalizedLogin, CancellationToken ct)
        => q.Query().FirstOrDefaultAsync(x => x.NormalizedLogin == normalizedLogin, ct);
