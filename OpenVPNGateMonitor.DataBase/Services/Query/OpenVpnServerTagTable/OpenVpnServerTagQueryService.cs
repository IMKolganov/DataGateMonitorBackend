...
public class OpenVpnServerTagQueryService(
    IQueryService<OpenVpnServerTag, int> q,
    IUnitOfWork uow) : IOpenVpnServerTagQueryService
{
    public Task<List<OpenVpnServerTag>> GetAll(CancellationToken ct)
        => q.GetAll(ct: ct);

    public Task<List<OpenVpnServerTag>> GetListByVpnServerId(int vpnServerId, CancellationToken ct)
        => q.Where(x => x.VpnServerId == vpnServerId, ct: ct);

    public Task<List<OpenVpnServerTag>> GetListByTagId(int tagId, CancellationToken ct)
