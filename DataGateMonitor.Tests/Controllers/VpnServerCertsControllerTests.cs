...
public class VpnServerCertsControllerTests
{
    private readonly Mock<ICertApiClient> _certApiClient = new();
    private readonly Mock<ILogger<VpnServerCertsController>> _logger = new();
    private readonly Mock<IVpnServerAccessQueryService> _vpnAccess = new();

    public VpnServerCertsControllerTests()
    {
        // Ensure Mapster mapping is registered for tests
        new VpnServerCertificateMapping().Register(TypeAdapterConfig.GlobalSettings);
    }

    private VpnServerCertsController CreateController()
    {
        var c = new VpnServerCertsController(_certApiClient.Object, _logger.Object, _vpnAccess.Object);
