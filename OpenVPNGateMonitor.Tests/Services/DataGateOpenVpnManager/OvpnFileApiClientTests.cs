...
public class OvpnFileApiClientTests
{
    ...
    {
        var httpFactory = new Mock<IHttpClientFactory>(MockBehavior.Loose);
        var server = new OpenVpnServer { Id = 1, ApiUrl = null!, ServerName = "S1", CreateDate = DateTimeOffset.UtcNow, LastUpdate = DateTimeOffset.UtcNow };
        var serverQuery = new Mock<IOpenVpnServerQueryService>(MockBehavior.Strict);
    ...
    public async Task AddOvpnFile_WhenSuccess_ReturnsMetadata()
    {
        ...
        var server = new OpenVpnServer { Id = 1, ApiUrl = "https://vpn.test/", ServerName = "S1", CreateDate = DateTimeOffset.UtcNow, LastUpdate = DateTimeOffset.UtcNow };
        var serverQuery = new Mock<IOpenVpnServerQueryService>(MockBehavior.Strict);
        serverQuery.Setup(q => q.GetById(1, It.IsAny<CancellationToken>())).ReturnsAsync(server);
