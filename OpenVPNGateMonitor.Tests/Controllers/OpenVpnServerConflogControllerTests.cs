...
public class OpenVpnServerConflogControllerTests
{
    private readonly Mock<IOpenVpnServerConflogService> _conflogService = new();
    private readonly Mock<IOpenVpnServerConflogQueryService> _conflogQuery = new();
    private readonly OpenVpnServerConflogController _controller;

    public OpenVpnServerConflogControllerTests()
    {
        _controller = new OpenVpnServerConflogController(_conflogService.Object, _conflogQuery.Object);
    }

    [Fact]
    public async Task FetchAndSave_WhenChanged_Returns_Ok_WithItem()
    {
        var entity = new OpenVpnServerConflog { Id = 1, VpnServerId = 5, RequestUrl = "https://x", PayloadJson = "{}" };
        ...
        var response = Assert.IsType<ApiResponse<OpenVpnServerConflogResponse?>>(ok.Value);
    ...
    {
        var entity = new OpenVpnServerConflog { Id = 2, VpnServerId = 10, RequestUrl = "https://srv", PayloadJson = "{}" };
    ...
    {
        var entity = new OpenVpnServerConflog { Id = 7, RequestUrl = "https://x", PayloadJson = "{}", CreateDate = DateTimeOffset.UtcNow };
    ...
    {
        _conflogQuery.Setup(q => q.GetById(99, It.IsAny<CancellationToken>())).ReturnsAsync((OpenVpnServerConflog?)null);
    ...
    {
        var paged = new OpenVPNGateMonitor.SharedModels.Responses.PagedResponse<OpenVpnServerConflog>
        {
            Page = 1,
            PageSize = 20,
            TotalCount = 0,
            Items = new List<OpenVpnServerConflog>()
        };
        ...
        var ok = Assert.IsType<OkObjectResult>(result.Result);
        var response = Assert.IsType<ApiResponse<OpenVpnServerConflogPageResponse>>(ok.Value);
        Assert.True(response.Success);
        Assert.NotNull(response.Data);
