...
public class OpenVpnFrontendHub(OpenVpnMicroserviceClient proxy, ILogger<OpenVpnFrontendHub> logger) : Hub
{
    public override async Task OnConnectedAsync()
    {
        ...
        {
            logger.LogInformation("Client connected: {ConnectionId} joined group {ServerId}", Context.ConnectionId, serverId);
    ...
    public override async Task OnDisconnectedAsync(Exception? exception)
    {
        ...
        if (int.TryParse(serverIdStr, out var serverId))
        {
            logger.LogInformation("Client disconnected: {ConnectionId} left group {ServerId}", Context.ConnectionId, serverId);
    ...
    public async Task SendCommand(string command)
    {
        ...
        logger.LogInformation("Received command '{Command}' for serverId={ServerId} from {ConnectionId}", command, serverId, Context.ConnectionId);
